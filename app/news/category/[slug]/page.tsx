...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params; const category=newsCategories.find(item=>item.slug===slug); if(!category)return {}; return pageMetadata(`${category.name}资讯`,`${category.name}领域的AI电商出海动态与编辑解读。`,`/news/category/${slug}`);}
